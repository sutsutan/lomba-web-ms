...
const NewsArchive = () => {
  ...
  const categories = useMemo(() => {
    const unique = Array.from(
      new Set(allNewsData.map((news) => news.category).filter(Boolean))
    );
    return unique;
  }, [allNewsData]);

  const filteredNewsData = useMemo(() => {
    if (selectedCategory === "all") return allNewsData;
    return allNewsData.filter((news) => news.category === selectedCategory);
  }, [allNewsData, selectedCategory]);

  const totalPages = useMemo(() => {
    return Math.ceil(filteredNewsData.length / itemsPerPage);
  }, [filteredNewsData]);

  const currentNews = useMemo(() => {
    const start = (currentPage - 1) * itemsPerPage;
    return filteredNewsData.slice(start, start + itemsPerPage);
  }, [filteredNewsData, currentPage]);
