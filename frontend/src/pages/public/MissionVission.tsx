...
const TiltCard = ({ misi }: { misi: any }) => {
  ...
  return (
    ...
    >
      <div style={{ transform: "translateZ(20px)" }} className="absolute -bottom-10 -right-10 w-48 h-48 opacity-[0.03] grayscale pointer-events-none group-hover:scale-110 transition-transform duration-700">
      ...
      <div style={{ transform: "translateZ(50px)" }} className="relative z-10">
        ...
        <h3 className="text-2xl font-bold text-[#0F5F58] mb-4 group-hover:text-[#2D8B82] transition-colors">{misi.title}</h3>
...
const VisionMission = () => {
  ...
  useEffect(() => {
    const fetchHero = async () => {
      try {
        ...
        const filtered = data.filter((i: any) => i.category === 'vision-mission' && i.is_active);
        
        setHeroSlides(filtered.map((i: any) => ({
          image_url: i.image_url,
          title: language === 'id' ? i.title_id : i.title_en,
          subtitle: language === 'id' ? i.subtitle_id : i.subtitle_en
        })));
  ...
  const missions = [
    { icon: <Rocket className="w-6 h-6 text-white" />, title: t("misi1_title"), desc: t("misi1_desc"), color: "bg-[#0F5F58]" },
    { icon: <Users className="w-6 h-6 text-white" />, title: t("misi2_title"), desc: t("misi2_desc"), color: "bg-[#2D8B82]" },
    { icon: <Globe className="w-6 h-6 text-white" />, title: t("misi3_title"), desc: t("misi3_desc"), color: "bg-[#B8C5D0]" },
    { icon: <TrendingUp className="w-6 h-6 text-white" />, title: t("misi4_title"), desc: t("misi4_desc"), color: "bg-[#0F5F58]" }
  ];
              ...
              <ScrollReveal direction="left">
                <div className="space-y-8">
                  ...
                  <div className="relative">
                    ...
                    <p className="text-2xl md:text-3xl text-gray-700 font-medium leading-relaxed italic pl-4">{t("vision_quote")}</p>
                  </div>
                </div>
              </ScrollReveal>
