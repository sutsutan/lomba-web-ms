  ...
  return (
    <section className="section-padding bg-section overflow-hidden">
      <div className="container mx-auto px-4 md:px-8">
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between mb-8 md:mb-16 lg:mb-24 px-2 md:px-12">
          <ScrollReveal>
            <div className="text-left">
              <h2 className="section-title !mx-0 text-2xl md:text-3xl lg:text-4xl">Moments of Achievement</h2>
              <p className="text-muted-foreground mt-2 text-sm md:text-base">Celebrating our students' global excellence</p>
            </div>
          </ScrollReveal>
          ...
          <ScrollReveal delay={0.2}>
            <div className="flex items-center gap-2 mt-6 md:mt-0">
              <div className="relative flex items-center justify-center h-16 w-24 md:h-20 md:w-32">
                {/* Garis miring */}
                <div className="absolute h-12 md:h-16 w-[2px] bg-primary/30 rotate-[45deg] z-10" />

                {/* Button Prev */}
                <button
                  onClick={prevSlide}
                  className="absolute left-1 md:left-2 -top-1 w-10 h-10 md:w-12 md:h-12 border-2 border-primary/40 flex items-center justify-center rotate-45 hover:bg-primary hover:text-white transition-all group"
                >
                  <ChevronLeft className="-rotate-45 w-5 h-5 md:w-6 md:h-6 group-active:-translate-x-1 transition-transform" />
                </button>

                {/* Button Next */}
                <button
                  onClick={nextSlide}
                  className="absolute right-1 md:right-2 -bottom-1 w-10 h-10 md:w-12 md:h-12 border-2 border-primary/40 flex items-center justify-center rotate-45 hover:bg-primary hover:text-white transition-all group"
                >
                  <ChevronRight className="-rotate-45 w-5 h-5 md:w-6 md:h-6 group-active:translate-x-1 transition-transform" />
                </button>
              </div>
            </div>
          </ScrollReveal>
        ...
        <div className="relative w-full max-w-6xl mx-auto">
          ...
          {/* Layer Fade Smooth di Samping - hanya di desktop */}
          <div className="hidden md:block absolute inset-y-0 left-0 w-12 md:w-32 bg-gradient-to-r from-section via-section/80 to-transparent z-20 pointer-events-none" />
          <div className="hidden md:block absolute inset-y-0 right-0 w-12 md:w-32 bg-gradient-to-l from-section via-section/80 to-transparent z-20 pointer-events-none" />
            ...
            {achievements.map((_, index) => (
              <button
                ...
                className={`transition-all duration-500 rounded-full ${
                  index === currentIndex ? 'w-8 md:w-10 h-2.5 md:h-3 bg-primary' : 'w-2.5 md:w-3 h-2.5 md:h-3 bg-slate-300'
                }`}
              />
            ))}
          </div>
